...
import { CsvParserService } from '@/lib/services/projects/importing/parsers/csv.parser';
...
export async function POST(request: NextRequest) {
    try {
        ...
        const parsed = CsvParserService.parseCsv(content, options);

        if (parsed.entries.length === 0) {
            return NextResponse.json(
                { error: 'No valid entries found in CSV content', warnings: parsed.metadata.parseWarnings },
                { status: 422 }
            );
        }
        ...
        return NextResponse.json(
            { error: 'Failed to parse CSV content', details: error instanceof Error ? error.message : 'Unknown error' },
            { status: 500 }
        );
    }
}
