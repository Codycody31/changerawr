...
/**
 * @method DELETE
 * @description Permanently deletes every changelog entry for a project. The project and its settings are kept.
 * @query {
 ...
 *     "entriesDeleted": { "type": "number" }
 *   }
 * }
 * @error 403 Forbidden - Only administrators can delete all entries
 * @error 404 Changelog not found
 * @error 500 An unexpected error occurred while deleting changelog entries
 */
export async function DELETE(
    ...
    try {
        ...
        const { count } = await db.changelogEntry.deleteMany({
            where: { changelogId: changelog.id }
        })

        try {
            await createAuditLog(
                'DELETE_ALL_CHANGELOG_ENTRIES',
                user.id,
                user.id,
                {
                    projectId,
        ...
        return NextResponse.json({ success: true, entriesDeleted: count })
    } catch (error) {
        console.error('Error deleting changelog entries:', error)
        return NextResponse.json(
            { error: 'Failed to delete changelog entries' },
            { status: 500 }
        )
    }
}
