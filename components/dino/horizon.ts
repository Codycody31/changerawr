...
import type {ConfigProvider} from './game_config';
...
export class Horizon {
    ...
    private updateClouds(deltaTime: number, speed: number) {
        // When cloudFrequency is 0 (ambient mode) skip entirely — including the
        // "add one if array is empty" branch inside updateBackgroundEl.
        if (this.cloudFrequency <= 0) return;
    ...
    /**
     * Remove all existing clouds and prevent new ones from spawning.
     * Called in ambient mode where clouds are disabled.
     */
    clearClouds() {
        this.clouds = [];
        this.cloudFrequency = 0;
    }
