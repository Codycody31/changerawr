...
import type {CollisionBox} from './offline_sprite_definitions';
...
export class DistanceMeter {
    ...
    /**
     * Set the highscore from raw pixel distance (converts to display units internally).
     */
    ...
    /**
     * Set the highscore directly from a display-unit score (no conversion).
     */
    setHighScoreDisplay(displayScore: number) {
        if (displayScore <= 0) return;
        const highScoreStr =
            (this.defaultString + Math.ceil(displayScore)).substr(-this.maxScoreUnits);
        this.highScore = 'HI ' + highScoreStr;
    }
