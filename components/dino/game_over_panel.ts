...
export class GameOverPanel {
    ...
    private drawRestartButton() {
        ...
        // Clear previous frame — drawImage leaves transparent source pixels intact,
        // so without this each animation frame accumulates on the canvas.
        this.canvasCtx.clearRect(
            restartTargetX, restartTargetY,
            dimensions.restartWidth, dimensions.restartHeight);

        // Clamp to the last valid frame — the animation loop increments currentFrame
        // past frames.length as a sentinel; without clamping the drawImage call gets
        // NaN coordinates and the button disappears after clearRect ran.
        const frameIdx = Math.min(this.currentFrame, animConfig.frames.length - 1);
        framePosX = animConfig.frames[frameIdx]!;
