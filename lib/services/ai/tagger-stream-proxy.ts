...
/**
 * Server-Sent Events proxy for the changelog-tagger's
 * /api/v1/training/stream endpoint. Proxied (rather than hit directly from
 * the browser) because the tagger may not be publicly reachable and its API
 * key must stay server-side.
 */
export async function proxyTaggerStream(): Promise<NextResponse> {
  ...
  try {
    ...
    // long as the caller holds the connection. A hard deadline would kill a
    // perfectly healthy connection and force the browser's EventSource into
    // an endless reconnect loop. The upstream sends its own keep-alive
